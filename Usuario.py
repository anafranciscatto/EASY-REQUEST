    ...
    def cadastrar(self, cpf, nome, email, senha, sn, foto, permissao, id_funcao): # Função que cadastra o usuário no Banco de Dados.
        try:
            ...
            if foto:
                ...
                mycursor.execute(f"INSERT INTO tb_funcionarios (CPF_funcionario, nome, email, senha, SN, permissao, id_funcao) VALUES ('{cpf}','{nome}', '{email}', '{senha_criptografada}', '{sn}', '{permissao}', {id_funcao});")
            myBD.commit()
